...
@app.route('/add_customer/<governorate>', methods=['GET', 'POST'])
...
def edit_customer(customer_id):
    ...
    conn.execute('''
        UPDATE customers 
        SET company_name=?, address=?, contact_person=?, mobile1=?, mobile2=?, phone=?, last_updated=?
        WHERE id=?
    ''', (company_name, address, contact_person, mobile1, mobile2, phone, datetime.now(), customer_id))
...
def add_communication(customer_id):
    ...
    conn.execute('''
        ...
        VALUES (?, ?, ?)
    ''', (customer_id, communication_details, datetime.now()))
    conn.execute('UPDATE customers SET last_updated = ? WHERE id = ?', (datetime.now(), customer_id))
    conn.commit()
    conn.close()
...
@app.route('/upload_file/<int:customer_id>', methods=['GET', 'POST'])
def upload_file(customer_id):
    ...
    if 'file' not in request.files:
        ...
        conn.close()
        return redirect(url_for('upload_file', customer_id=customer_id))
    ...
    try:
        ...
        conn.execute('''
            INSERT INTO customer_files (customer_id, filename, original_filename, file_description, file_size, upload_date)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (customer_id, filename, original_filename, file_description, file_size, datetime.now()))
...
def download_file(file_id):
    """Download file"""
    conn = get_db_connection()
    file_info = conn.execute('SELECT * FROM customer_files WHERE id = ?', (file_id,)).fetchone()
    conn.close()
...
def delete_file(file_id):
    ...
    try:
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
        conn.execute('DELETE FROM customer_files WHERE id = ?', (file_id,))
        conn.commit()
...
def search():
    ...
    conn = get_db_connection()
    customers = conn.execute('''
        SELECT c.*, COUNT(cl.id) as communication_count, MAX(cl.log_date) as last_communication
        FROM customers c
        LEFT JOIN communication_logs cl ON c.id = cl.customer_id
    ...
    return render_template('search_results.html', customers=customers, query=query)
...
def statistics():
    ...
    gov_stats = []
    for gov in GOVERNORATES:
        count = conn.execute('SELECT COUNT(*) as count FROM customers WHERE governorate = ?', (gov,)).fetchone()['count']
